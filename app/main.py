...
from gb_module import GradientBoosting 
import json
...
@app.post("/gradient-boosting")
def calculo(samples: int, n_estimators: int, learning_rate: float):
    output_file = 'gradient_boosting.png'
    ...
    #learning_rate = 0.1
    #n_estimators = 50
    gb = GradientBoosting(learning_rate, n_estimators)
    gb.fit(X, y)
    y_pred = gb.predict(X)
    ...
    plt.xlabel("X")
    plt.ylabel("y")
    plt.title("Regresión con Gradient Boosting")
    plt.legend()
...
@app.get("/gradient-boosting-graph")
